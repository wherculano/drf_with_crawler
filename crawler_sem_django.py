    ...
    def access_url(self):
        ...
        for elem in div_caption:
            if self.modelo.lower() in elem.text.lower():
                for link in elem.find_elements_by_tag_name('a'):
                    if self.modelo.lower() in link.text.lower():
        ...
        for l in links:
            ...
            for btn in btn_hdd:
                ...
                conteudo.append({
                    'modelo': modelo, 'descricao': descricao, 'valor': float(valor_principal), 'nota': nota,
                    'hdd': dct_hdds,
        ...
        driver.quit()
        return json.dumps(sorted(conteudo, key=lambda x: x['valor'], reverse=False), indent=4, sort_keys=False,
                          ensure_ascii=False)
